...
async function handleLive(req, res, url) {
  ...
  const scenario = url.searchParams.get('scenario') === 'meeting1' ? 'meeting1' : 'meeting2';
  ...
  const session = carry.createSession({
    ...
    metadata: { meeting_type: 'client_meeting', demo: true, scenario },
  });
  ...
  try {
    await streamChunks(getLawyerScenario(scenario), async (chunk) => {
      if (closed) return;
